...
| 选牌匹配奖励 | 0.4（默认） | LLM 推荐 vs Agent 实际选牌 | 稠密，构筑层面引导 |
| 遗物匹配奖励 | 0.25（默认） | LLM 推荐遗物 vs Agent 实际选择 | 稠密，构筑层面引导 |
| 路线匹配奖励 | 0.25（默认） | LLM 路线评估 vs Agent 实际走位 | 稠密，路线层引导 |
| 战斗开局匹配奖励 | 0.2（默认） | LLM opening 序列 vs 前几步实际出牌 | 稠密，战斗起手引导 |
